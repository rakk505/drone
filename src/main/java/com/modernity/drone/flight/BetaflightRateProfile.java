...
public record BetaflightRateProfile(AxisRate roll, AxisRate pitch, AxisRate yaw) {
    ...
    public record AxisRate(double rcRate, double superRate, double expo) {
        public AxisRate {
            rcRate = FlightMath.clamp(FlightMath.finiteOr(rcRate, 1.0), 0.0, 3.0);
            superRate = FlightMath.clamp(FlightMath.finiteOr(superRate, 0.0), 0.0, 1.0);
        ...
        public double radiansPerSecond(double stick) {
            ...
            double shaped = input * Math.pow(absoluteInput, 3.0) * expo + input * (1.0 - expo);
            ...
            degreesPerSecond /= Math.max(0.01, 1.0 - absoluteInput * superRate);
